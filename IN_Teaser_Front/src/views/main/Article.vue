...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      save() {   
        // user_seq , user_nickname,  feed_subject (말머리), feed_contents , feed_hashtag , feed_scope 
        var hashtag = []
        ...
        if (isSave) {
          FeedApi.createFeed(data, res=> {
            alert('성공적으로 글이 작성되었습니다.')
            this.$router.push({name:'Main'})
          }, err => {
            alert('실패했습니다.')
          })
          this.viewerText = this.$refs.articleEditor.invoke('getHtml')
        } else {
          alert('빈 값이 존재합니다.')
        }
